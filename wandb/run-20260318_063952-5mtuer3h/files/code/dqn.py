...
class Args:
    ...
    data_path: str = 'nvda_stock_data.csv'
    """the path to your stock data CSV"""
    window_size: int = 50
    """how many days of history the AI sees at once"""
...
def make_env(env_id, seed, idx, capture_video, run_name, data_path):
    def thunk():
        ...
        env.unwrapped.add_metric('position', lambda history: history['position', -1])
        env.unwrapped.add_metric('portfolio_valuation', lambda history: history['portfolio_valuation', -1])
...
if __name__ == "__main__":
    args = tyro.cli(Args)
    #please just track my logs i beg of you
    args.track = True
    #add yfinance for more stock data
    if not os.path.exists(args.data_path):
        print(f'Download data for {args.data_path}...')
        df_download = yf.download('SPY', period='5y', interval='1d')

        #flatten value
        if isinstance(df_download.columns, pd.MultiIndex):
            df_download.columns = df_download.columns.get_level_values(0)

        df_download.to_csv(args.data_path)
    ...
    envs = gym.vector.SyncVectorEnv(
        [make_env(args.env_id, args.seed + i, i, args.capture_video, run_name, args.data_path) for i in range(args.num_envs)]
    )
    ...
    for global_step in range(args.total_timesteps):
        ...
        print([k for k in infos.keys() if 'data' in k.lower() or 'price' in k.lower() or 'close' in k.lower()])

        if 'portfolio_valuation' in infos:
            portfolio_val = float(infos['portfolio_valuation'][0])
            stock_price = float(infos['data_close'][0]) if 'data_close' in infos else None

            log_dict = {
                "performance/portfolio_val": portfolio_val,
            ...
            if stock_price is not None:
                log_dict["performance/stock_price"] = stock_price
        ...
        for i in range(args.num_envs):
            ...
            if isinstance(infos, dict):
                # SyncVectorEnv stores per-env info in infos as arrays or under '_' masks
                for key in ['Portfolio Val', 'data_close', 'position']:
                    if key in infos:
                        val = infos[key]
            ...
            if 'Portfolio Val' in env_info and 'data_close' in env_info:
                current_price = env_info['data_close']
                current_val = env_info['Portfolio Val']
                ...
                break  # log env_0 only to avoid noise
        # ALGO LOGIC: training.
